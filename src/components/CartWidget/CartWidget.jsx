...
const CartWidget = ({ setMenuIsOpen, value }) => {
	const { carrito, deleteItem, totalPrice, cartLength } =
	...
	return (
		<div>
			...
			<section className="cartContainer" id="cartContainer">
				<div
					className={`cartContainer__overlay ${
						value[1] ? "displayCartContainer" : ""
					}`}
				></div>
				<div
					className={`cartContainer__content ${
						value[1] ? "displayCartContainer" : ""
					}`}
				>
					{carrito.length !== 0 && (
						<h2 className="cartContainer--title">Añadido a su Carrito</h2>
					)}
					<ul className="cartContainer__list">
						{carrito.length === 0 ? (
							<li className="cartContainer__vacio">
								...
								<Link to="/">
									<button
										className="cartContainer__vacio--button"
										onClick={handleCloseCartWidget}
									>
										Seguir comprando
									</button>
								</Link>
								...
								{carrito.map(prod => {
									...
									return (
										<CSSTransition
											key={id}
											timeout={500}
											classNames="itemGroupCartWidget"
										>
